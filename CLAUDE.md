...
**50-move rule:** `GameController.halfmoveClock` counts half-moves since the last pawn move or capture (castling doesn't reset it, matching the real rule); it resets to 0 on either, increments otherwise, and hitting 100 (50 full moves) ends the game as a draw. Checkmate/stalemate are still checked first each turn, since either takes priority over this counter. No threefold-repetition detection exists yet.
